...
# Check if there were multiple delays on the same line in the same time bin
print(disrupt[["time_bin", "color"]].duplicated().value_counts())

# Mark total delay time on each line in each time bin
disrupt["delaytime"] = disrupt.groupby(["time_bin", "color"]).delay.transform('sum')

# Keep only necessary variables
disrupt = disrupt[["time_bin", "red", "orange", "yellow", "green", "blue", "silver", "delaytime"]]
disrupt.drop_duplicates(inplace=True)

# Merge by time and line
